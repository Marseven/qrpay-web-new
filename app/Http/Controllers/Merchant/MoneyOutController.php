   ...
   public function confirmMoneyOut(Request $request){
    ...
    $get_values = $this->placeValueWithFields($payment_fields,$payment_field_validate);
        try{
            ...
            return redirect()->route("merchant.withdraw.index")->with(['success' => ['Withdraw money request send to admin successful']]);
        }catch(Exception $e) {
        ...
        if (curl_errno($ch)) {
            ...
            if($result['status'] && $result['status'] == 'success'){
                try{
                    ...
                    $this->insertDeviceManual($moneyOutData,$inserted_id);
                    session()->forget('moneyoutData');
                    return redirect()->route("merchant.withdraw.index")->with(['success' => ['Withdraw money request send successful']]);
                }catch(Exception $e) {
                    return back()->with(['error' => [$e->getMessage()]]);
                }
