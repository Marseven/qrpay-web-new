...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function sendVerifyCode(Request $request){
        ...
        return redirect()->route('user.email.verify',$data['token'])->with(['success' => ['Varification code send to your email successfully!']]);
    }
    public function verifyCode(Request $request,$token){
        ...
        if($auth_column->created_at->addSeconds($otp_exp_sec) < now()) {
            $auth_column->delete();
            return redirect()->route('user.register')->with(['error' => ['Session expired. Please try again']]);
        }
        try{
            $auth_column->delete();
        }catch(Exception $e) {
            return redirect()->route('user.register')->with(['error' => ['Something went worng! Please try again']]);
        }
    ...
    public function resendCode(){
        ...
        return redirect()->route('user.email.verify',$data['token'])->with(['success' => ['Varification code resend success!']]);
    }
