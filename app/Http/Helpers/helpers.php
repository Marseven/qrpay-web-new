...
function getFlutterwaveBanks($secret_key = null){
    $cardApi = VirtualCardApi::first();
    $secretKey = $secret_key??$cardApi->secret_key;
    $curl = curl_init();

    curl_setopt_array($curl, array(
      CURLOPT_URL => $cardApi->url.'/banks/NG',
...
function checkBankAccount($secret_key = null,$account_number,$bank_code){
    $cardApi = VirtualCardApi::first();
    $secretKey = $secret_key??$cardApi->secret_key;
    $ch = curl_init();
    $url =  $cardApi->url.'/accounts/resolve';
...
function flutterwaveBalance($secret_key = null){
    ...
    curl_setopt_array($curl, array(
        ...
        CURLOPT_URL =>  $cardApi->url.'/balances',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => "",
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
