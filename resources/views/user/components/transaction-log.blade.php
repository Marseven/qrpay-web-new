            ...
            <div class="dashboard-list-item sent">
                        ...
                        <div class="dashboard-list-user-content">
                            ...
                            @elseif ($item->type == payment_gateway_const()::TYPEMONEYOUT)
                                <h4 class="title">{{ __('Withdraw Money') }} <span
                                        class="text--warning">{{ $item->currency->gateway->name }}</span></h4>
                            ...
                            @elseif ($item->type == payment_gateway_const()::TICKETPAY)
                                <h4 class="title">{{ __('Ticket Pay') }} {{ @$item->details->trx_id }}<span
                            ...
                            @elseif ($item->type == payment_gateway_const()::TYPEMONEYEXCHANGE)
                                <h4 class="title">{{ __('Exchange Money') }} <span
                                        class="text--warning">{{ $item->details->request_currency }} To
                                        {{ $item->details->exchange_currency }}</span></h4>
                            @elseif ($item->type == payment_gateway_const()::TYPEADDSUBTRACTBALANCE)
                                <h4 class="title">
                                    {{ __('Balance Update From Admin (' . $item->user_wallet->currency->code . ')') }}
                                </h4>
                            @elseif ($item->type == payment_gateway_const()::TYPETRANSFERMONEY)
                                @if ($item->isAuthUser())
                                    @if ($item->attribute == payment_gateway_const()::SEND)
                                        <h4 class="title">
                                            {{ __('Send Money to @' . @$item->details->receiver->username . ' (' . @$item->details->receiver->email . ')') }}
                                        </h4>
                                    @elseif ($item->attribute == payment_gateway_const()::RECEIVED)
                                        <h4 class="title">
                                            {{ __('Received Money from @' . @$item->details->sender->username . ' (' . @$item->details->sender->email . ')') }}
                                        </h4>
                                    @endif
                                @endif
                            @elseif ($item->type == payment_gateway_const()::TYPEMAKEPAYMENT)
                                @if ($item->isAuthUser())
                                    @if ($item->attribute == payment_gateway_const()::SEND)
                                        <h4 class="title">
                                            {{ __('Make Payment to @' . @$item->details->receiver->username . ' (' . @$item->details->receiver->email . ')') }}
                                        </h4>
                                    @elseif ($item->attribute == payment_gateway_const()::RECEIVED)
                                        <h4 class="title">
                                            {{ __('Make Payment from @' . @$item->details->sender->username . ' (' . @$item->details->sender->email . ')') }}
                                        </h4>
                                    @endif
                                @endif
                            @elseif ($item->type == payment_gateway_const()::SENDREMITTANCE)
                                @if ($item->isAuthUser())
                                    @if ($item->attribute == payment_gateway_const()::SEND)
                                        <h4 class="title">
                                            {{ __('Send Remittance to @' . $item->details->receiver->firstname . ' ' . @$item->details->receiver->lastname . ' (' . @$item->details->receiver->mobile_code . @$item->details->receiver->mobile . ')') }}
                                        </h4>
                                    @elseif ($item->attribute == payment_gateway_const()::RECEIVED)
                                        <h4 class="title">
                                            {{ __('Received Remittance from @' . @$item->details->sender->fullname . ' (' . @$item->details->sender->full_mobile . ')') }}
                                        </h4>
                                    @endif
                                @endif
                            @endif
                ...
                <div class="dashboard-list-right">
                    @if ($item->type == payment_gateway_const()::TYPEADDMONEY)
                        ...
                        <h6 class="exchange-money fw-bold">{{ get_amount($item->payable, $item->currency->currency_code) }}
                        </h6>
                    @elseif($item->type == payment_gateway_const()::TYPEMONEYOUT)
                        <h6 class="exchange-money text--warning fw-bold">
                            {{ get_amount($item->request_amount, get_default_currency_code()) }}</h6>
                        <h4 class="main-money ">{{ get_amount($item->payable, $item->currency->currency_code) }}</h4>
                    @elseif($item->type == payment_gateway_const()::BILLPAY)
                        <h4 class="main-money text--warning">
                            {{ get_amount($item->request_amount, get_default_currency_code()) }}</h4>
                        <h6 class="exchange-money fw-bold">{{ get_amount($item->payable, get_default_currency_code()) }}
                        </h6>
                    @elseif($item->type == payment_gateway_const()::TICKETPAY)
                        <h4 class="main-money text--warning">
                            {{ get_amount($item->request_amount, get_default_currency_code()) }}</h4>
                    ...
                    @elseif ($item->type == payment_gateway_const()::TYPEMONEYEXCHANGE)
                        <h4 class="main-money text--base">
                            {{ get_amount($item->request_amount, $item->user_wallet->currency->code) }}</h4>
                        <h6 class="exchange-money">
                            {{ get_amount($item->available_balance, $item->user_wallet->currency->code) }}</h6>
                    @elseif ($item->type == payment_gateway_const()::TYPEADDSUBTRACTBALANCE)
                        <h4 class="main-money text--base">
                            {{ get_amount($item->request_amount, $item->user_wallet->currency->code) }}</h4>
                        <h6 class="exchange-money">
                    ...
                    @elseif ($item->type == payment_gateway_const()::TYPETRANSFERMONEY)
                        @if ($item->attribute == payment_gateway_const()::SEND)
                            <h6 class="exchange-money text--warning ">
                                {{ get_amount($item->request_amount, get_default_currency_code()) }}</h6>
                            <h4 class="main-money fw-bold">{{ get_amount($item->payable, get_default_currency_code()) }}
                            </h4>
                        @elseif ($item->attribute == payment_gateway_const()::RECEIVED)
            ...
            <div class="preview-list-wrapper">
                ...
                @if ($item->type != payment_gateway_const()::TYPETRANSFERMONEY)
                    @if ($item->type != payment_gateway_const()::TYPEMAKEPAYMENT)
                        @if ($item->type != payment_gateway_const()::BILLPAY)
                            @if ($item->type != payment_gateway_const()::TICKETPAY)
                                        ...
                                        @if ($item->type != payment_gateway_const()::SENDREMITTANCE)
                                            <div class="preview-list-item">
                                                ...
                                                <div class="preview-list-right">
                                                    ...
                                                    @elseif ($item->type == payment_gateway_const()::SENDREMITTANCE)
                                                        <span>1 {{ get_default_currency_code() }} =
                                                            {{ get_amount($item->details->to_country->rate, $item->details->to_country->code) }}</span>
                                                    @elseif ($item->type == payment_gateway_const()::TYPEMONEYOUT)
                                                        <span>1 {{ get_default_currency_code() }} =
                                                            {{ get_amount($item->currency->rate, $item->currency->currency_code) }}</span>
                                                    @elseif ($item->type == payment_gateway_const()::TYPEMONEYEXCHANGE)
                                                        <span>1 {{ $item->user_wallet->currency->code }} =
                                                            {{ get_amount($item->details->exchange_rate, $item->details->exchange_currency) }}</span>
                                                    @elseif ($item->type == payment_gateway_const()::TYPEADDSUBTRACTBALANCE)
                                                        <span>1 {{ get_default_currency_code() }} =
                                                            {{ get_amount($item->user_wallet->currency->rate, $item->user_wallet->currency->code) }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                        @endif
                    ...
                    <div class="preview-list-item">
                        <div class="preview-list-left">
                            <div class="preview-list-user-wrapper">
                                ...
                                <div class="preview-list-user-content">
                                    <span>{{ __('Ticket Type') }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="preview-list-right">
                            <span class="text--base">{{ @$item->details->ticket_type_name }}</span>
                        </div>
                    </div>
                    <div class="preview-list-item">
                        <div class="preview-list-left">
                        ...
                        <div class="preview-list-right">
                            <span class="text--base">{{ @$item->details->ticket_number }}</span>
                        </div>
                    </div>
                @endif
                @if ($item->type == payment_gateway_const()::MOBILETOPUP)
                ...
                @if (
                    ...
                    @if ($item->attribute == payment_gateway_const()::SEND)
                        ...
                        <div class="preview-list-item">
                            ...
                            <div class="preview-list-right">
                                <span
                                    class="text--base">{{ get_amount($item->available_balance, get_default_currency_code()) }}</span>
                            </div>
                        </div>
                    @else
                        <div class="preview-list-item">
                                ...
                                @if ($item->type != payment_gateway_const()::SENDREMITTANCE)
                                    <div class="preview-list-item">
                                        ...
                                        <div class="preview-list-right">
                                            @if ($item->type == payment_gateway_const()::TYPEADDMONEY)
                                                <span
                                                    class="text-danger">{{ get_amount($item->available_balance, get_default_currency_code()) }}</span>
                                            @elseif ($item->type == payment_gateway_const()::TYPEMONEYOUT)
                                                @php
                                            ...
                                            @elseif ($item->type == payment_gateway_const()::VIRTUALCARD)
                                                <span class="fw-bold">
                                                    {{ get_amount(@$item->details->card_info->amount, get_default_currency_code()) }}</span>
                                            @elseif ($item->type == payment_gateway_const()::TYPEMONEYEXCHANGE)
                        ...
                        @if ($item->type != payment_gateway_const()::SENDREMITTANCE)
                            <div class="preview-list-item">
                                ...
                                <div class="preview-list-right">
                                    @if ($item->type == payment_gateway_const()::TYPEADDMONEY)
                                        <span
                                            class="text--warning">{{ get_amount($item->payable, $item->currency->currency_code) }}</span>
                                    @elseif ($item->type == payment_gateway_const()::TYPEMONEYOUT)
                                        <span
                                            class="text--danger">{{ get_amount($item->available_balance, get_default_currency_code()) }}</span>
                                    @elseif ($item->type == payment_gateway_const()::BILLPAY)
                                    ...
                                    @elseif ($item->type == payment_gateway_const()::TYPEMONEYEXCHANGE)
                                        <span
                                            class="text--warning">{{ get_amount($item->details->exchange_amount, $item->details->exchange_currency) }}</span>
                                    @elseif ($item->type == payment_gateway_const()::TYPEADDSUBTRACTBALANCE)
                ...
                @if ($item->type == payment_gateway_const()::VIRTUALCARD)
                    <div class="preview-list-item">
                        ...
                        <div class="preview-list-right">
                            <span
                                class="fw-bold">{{ get_amount($item->available_balance, get_default_currency_code()) }}</span>
                        </div>
                    </div>
                @endif
                @if ($item->type == payment_gateway_const()::SENDREMITTANCE)
                        ...
                        <div class="preview-list-item">
                            ...
                            <div class="preview-list-right">
                                <span class="text-base fw-bold">
                                    {{ number_format(@$item->details->recipient_amount, 2) }}
                                    {{ $item->details->to_country->code }}</span>
                            </div>
                        </div>
                    @endif
                    <div class="preview-list-item">
                        ...
                        <div class="preview-list-right">
                            <span class="text-base fw-bold">
                                {{ get_amount($item->available_balance, get_default_currency_code()) }}</span>
                        </div>
                    </div>
                @endif
                <div class="preview-list-item">
